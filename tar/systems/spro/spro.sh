...
kill_list=`ps -eF | grep spro | tr -s [:blank:] | cut -d ' ' -f 2 2>/dev/null` 2>/dev/null
stupid_count=0
...
rm ../../messages/spromessages 2>/dev/null
rm ./spro_fulllog 2>/dev/null
rm ../../messages/spro_kplog 2>/dev/null
rm ../../messages/spro_alive 2>/dev/null
...
do
	...
	touch ../../messages/spro_alive 2>/dev/null
					...
					then
						...
						then
							...
							then
								...
								then
									echo "Обнаружена БР с ID = "$id" с координатами "$x" и "$y" и скоростями "$vx" и "$vy >>../../messages/spromessages 2>/dev/null
									...
									echo "id = "$id >> ./spro_fulllog 2>/dev/null
									echo "`date +%s`:$id:$x:$y:0:-1" >> ../../messages/spro_kplog
								...
								then
									...
									then
										...
										let ammo=ammo-1
										echo "Произведен выстрел по новой цели БР с ID = "$id" с координатами "$x" и "$y" и скоростями "$vx" и "$vy >>../../messages/spromessages 2>/dev/null
										echo "`date +%s`:$id:$x:$y:1:$ammo" >> ../../messages/spro_kplog 2>/dev/null
										...
										echo "Нет снарядов. Вижу новую цель БР с ID = "$id" с координатами "$x" и "$y" и скоростями "$vx" и "$vy >>../../messages/spromessages 2>/dev/null
										echo "`date +%s`:$id:$x:$y:2:$ammo" >> ../../messages/spro_kplog 2>/dev/null
									fi
								fi
							fi
						fi
	...
	do
		...
		then
			...
			then
				...
				if [[ $flag_shot -eq 1 ]]
				then
					echo "Цель поражена ID = "$fired_id >> ../../messages/spromessages 2>/dev/null
					echo "`date +%s`:$fired_id:${fired[3+$((thiscount-1))*5]}:${fired[4+$((thiscount-1))*5]}:4:-1" >> ../../messages/spro_kplog 2>/dev/null
				else
					echo "Промах по цели ID = "$fired_id >>../../messages/spromessages 2>/dev/null
					echo "`date +%s`:$fired_id:${fired[3+$((thiscount-1))*5]}:${fired[4+$((thiscount-1))*5]}:5:-1" >> ../../messages/spro_kplog 2>/dev/null
				fi
	...
	echo $iter >> ./spro_idlog
	$((iter++)) 2>/dev/null
		
	sleep 0.5

	echo "___" >> ./spro_fulllog 2>/dev/null
	echo "NEW ITER"$iter >> ./spro_fulllog 2>/dev/null
	echo "___" >> ./spro_fulllog 2>/dev/null
