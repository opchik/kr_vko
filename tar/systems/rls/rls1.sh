...
kill_list=`ps -eF | grep rls1 | tr -s [:blank:] | cut -d ' ' -f 2 2>/dev/null` 2>/dev/null
stupid_count=0
...
rm ./rls1_idlog 2>/dev/null
rm ../../messages/rls1messages 2>/dev/null
rm ./rls1_fulllog 2>/dev/null
rm ../../messages/rls1_kplog 2>/dev/null
rm ../../messages/rls1_alive 2>/dev/null
...
function InSproDirection()
{
	...
	echo "thinking of theta before trunc= "$theta >> ./rls1_fulllog 2>/dev/null
	theta=(${theta/\,*})
	echo "thinking of theta after trunc= "$theta >> ./rls1_fulllog 2>/dev/null
	
	if [[ "$theta" = "-nan" ]]
	then
		echo "found tasty ananas" >> ./rls1_fulllog 2>/dev/null
		theta=180
	fi
	...
	echo "thetamax = "$thetaMin >> ./rls1_fulllog 2>/dev/null
	echo "thetamin = "$thetaMax >> ./rls1_fulllog 2>/dev/null
	...
	echo $check360 >> ./rls1_fulllog 2>/dev/null
	echo $check0 >> ./rls1_fulllog 2>/dev/null
	...
	then
		...
		then
			...
			echo "target in normal sector" >> ./rls1_fulllog 2>/dev/null
		
			echo "targetInSproDir = true" >> ./rls1_fulllog 2>/dev/null
			echo $phi >> ./rls1_fulllog 2>/dev/null
			echo $phiV >> ./rls1_fulllog 2>/dev/null
			return 1
		else
			echo "targetInSproDir = false" >> ./rls1_fulllog 2>/dev/null
			echo $phi >> ./rls1_fulllog 2>/dev/null
			echo $phiV >> ./rls1_fulllog 2>/dev/null
			return 0
		fi
		...
		then
			...
			echo "target in normal 360+sect" >> ./rls1_fulllog 2>/dev/null
		
			let thetaMax360=$thetaMax-360
			echo $thetaMax360 >> ./rls1_fulllog 2>/dev/null
			if [[ $phiV -gt $thetaMin || $phiV -lt $thetaMax360 ]]
			then
				echo "targetInSproDir = true" >> ./rls1_fulllog 2>/dev/null
				echo "phi = "$phi >> ./rls1_fulllog 2>/dev/null
				echo "phiV = "$phiV >> ./rls1_fulllog 2>/dev/null
				echo "thetaMax360 = "$thetaMax360 >> ./rls1_fulllog 2>/dev/null
				return 1
			else
				echo "targetInSproDir = true" >> ./rls1_fulllog 2>/dev/null
				echo "phi = "$phi >> ./rls1_fulllog 2>/dev/null
			...
			then
				...
				echo "target in normal 0-sect" >> ./rls1_fulllog 2>/dev/null
			
				let thetaMin0=360+$thetaMin
				echo $thetaMin0 >> ./rls1_fulllog 2>/dev/null
				if [[ $phiV -gt $thetaMin0 || $phiV -lt $thetaMax ]]
				then
					echo "targetInSproDir = true" >> ./rls1_fulllog 2>/dev/null
					echo "phi = "$phi >> ./rls1_fulllog 2>/dev/null
					echo "phiV = "$phiV >> ./rls1_fulllog 2>/dev/null
					echo "phiMin0 = "$thetaMin0 >> ./rls1_fulllog 2>/dev/null
					return 1
				else
					echo "targetInSproDir = true" >> ./rls1_fulllog 2>/dev/null
					echo "phi = "$phi >> ./rls1_fulllog 2>/dev/null
...
function InRlsZone()
{
	...
	then
		...
		echo "angle = "$phi >> ./rls1_fulllog 2>/dev/null
		echo "id = "$id >> ./rls1_fulllog 2>/dev/null
		echo "phimax = "$phiMax >> ./rls1_fulllog 2>/dev/null
		echo "phimin = "$phiMin >> ./rls1_fulllog 2>/dev/null
...
while :
do
	touch ../../messages/rls1_alive 2>/dev/null
		...
		then
			...
			then
				...
				do
					...
					if [[ $deadflag -eq 1 ]]
					then
						echo "checkid = "$id >> ./rls1_fulllog 2>/dev/null
						echo "thisid = "${targets[0+$((idx-1))*10]} >> ./rls1_fulllog 2>/dev/null
						echo "isalive = "${targets[5+$((idx-1))*10]} >> ./rls1_fulllog 2>/dev/null
						echo "isreported = "${targets[6+$((idx-1))*10]} >> ./rls1_fulllog 2>/dev/null
					...
					then
						...
						then
							...
							then
								echo "Обнаружена цель с ID = "$id" с координатами "$x" и "$y" и скоростями "$vx" и "$vy >>../../messages/rls1messages 2>/dev/null
								echo "`date +%s`:$id:$x:$y:0:-1" >> ../../messages/rls1_kplog 2>/dev/null
								...
								then
									...
									echo "id = "$id >> ./rls1_fulllog 2>/dev/null
									echo "targetInSproDir = "$targetInSproDir >> ./rls1_fulllog 2>/dev/null
									if [[ $targetInSproDir -eq 1 ]]
									then
										echo "Цель ID = "$id" движется в зону действия СПРО" >> ../../messages/rls1messages 2>/dev/null
										echo "`date +%s`:$id:$x:$y:3:-1" >> ../../messages/rls1_kplog 2>/dev/null
									fi
	...
	echo "unfound count = "$unfound_count >> ./rls1_fulllog 2>/dev/null
	
	# после полной итерации считывания целей обрабатываем новые цели	
	for ((thiscount=1; thiscount<=unfound_count; thiscount++))
	do
		echo "unfound id# "$thiscount" = "${unfound[0+$((thiscount-1))*3]} >> ./rls1_fulllog 2>/dev/null
	...
	echo $iter >> ./rls1_idlog
	$((iter++)) 2>/dev/null
		
	sleep 0.5

	echo "___" >> ./rls1_fulllog 2>/dev/null
	echo "NEW ITER"$iter >> ./rls1_fulllog 2>/dev/null
	echo "___" >> ./rls1_fulllog 2>/dev/null
