...
kill_list=`ps -eF | grep rls2 | tr -s [:blank:] | cut -d ' ' -f 2 2>/dev/null` 2>/dev/null
stupid_count=0
...
rm ./rls2_idlog 2>/dev/null
rm ../../messages/rls2messages 2>/dev/null
rm ./rls2_fulllog 2>/dev/null
rm ../../messages/rls2_kplog 2>/dev/null
rm ../../messages/rls2_alive 2>/dev/null

x0=12000000 #м 
y0=5000000 #м

az=135    #градусы азимут

ph=200     #градусы сектор обзора туда-обратно
...
function InSproDirection()
{
	...
	echo $dist2 >> ./rls2_fulllog 2>/dev/null
	
	# some magic trigonometry constructions
	theta=$(echo | awk " { x=$Rpsro/sqrt($dist2); y=atan2(x, sqrt(1-x*x))*180/3.141592653589; print y}")
	echo "thinking of theta before trunc= "$theta >> ./rls2_fulllog 2>/dev/null
	theta=(${theta/\,*})
	echo "thinking of theta after trunc= "$theta >> ./rls2_fulllog 2>/dev/null
	
	if [[ "$theta" = "-nan" ]]
	then
		echo "found tasty ananas" >> ./rls2_fulllog 2>/dev/null
		theta=180
	fi
	...
	echo "thetamax = "$thetaMin >> ./rls2_fulllog 2>/dev/null
	echo "thetamin = "$thetaMax >> ./rls2_fulllog 2>/dev/null
	...
	echo $check360 >> ./rls2_fulllog 2>/dev/null
	echo $check0 >> ./rls2_fulllog 2>/dev/null
	...
	then
		...
		then
			...
			echo "target in normal sector" >> ./rls2_fulllog 2>/dev/null
		
			echo "targetInSproDir = true" >> ./rls2_fulllog 2>/dev/null
			echo $phi >> ./rls2_fulllog 2>/dev/null
			echo $phiV >> ./rls2_fulllog 2>/dev/null
			return 1
		else
			echo "targetInSproDir = false" >> ./rls2_fulllog 2>/dev/null
			echo $phi >> ./rls2_fulllog 2>/dev/null
			echo $phiV >> ./rls2_fulllog 2>/dev/null
			return 0
		fi
		...
		then
			...
			echo "target in normal 360+sect" >> ./rls2_fulllog 2>/dev/null
		
			let thetaMax360=$thetaMax-360
			echo $thetaMax360 >> ./rls2_fulllog 2>/dev/null
			if [[ $phiV -gt $thetaMin || $phiV -lt $thetaMax360 ]]
			then
				echo "targetInSproDir = true" >> ./rls2_fulllog 2>/dev/null
				echo "phi = "$phi >> ./rls2_fulllog 2>/dev/null
				echo "phiV = "$phiV >> ./rls2_fulllog 2>/dev/null
				echo "thetaMax360 = "$thetaMax360 >> ./rls2_fulllog 2>/dev/null
				return 1
			else
				echo "targetInSproDir = true" >> ./rls2_fulllog 2>/dev/null
				echo "phi = "$phi >> ./rls2_fulllog 2>/dev/null
			...
			then
				...
				echo "target in normal 0-sect" >> ./rls2_fulllog 2>/dev/null
			
				let thetaMin0=360+$thetaMin
				echo $thetaMin0 >> ./rls2_fulllog 2>/dev/null
				if [[ $phiV -gt $thetaMin0 || $phiV -lt $thetaMax ]]
				then
					echo "targetInSproDir = true" >> ./rls2_fulllog 2>/dev/null
					echo "phi = "$phi >> ./rls2_fulllog 2>/dev/null
					echo "phiV = "$phiV >> ./rls2_fulllog 2>/dev/null
					echo "phiMin0 = "$thetaMin0 >> ./rls2_fulllog 2>/dev/null
					return 1
				else
					echo "targetInSproDir = true" >> ./rls2_fulllog 2>/dev/null
					echo "phi = "$phi >> ./rls2_fulllog 2>/dev/null
...
function InRlsZone()
{
	...
	then
		...
		echo "angle = "$phi >> ./rls2_fulllog 2>/dev/null
		echo "id = "$id >> ./rls2_fulllog 2>/dev/null
		echo "phimax = "$phiMax >> ./rls2_fulllog 2>/dev/null
		echo "phimin = "$phiMin >> ./rls2_fulllog 2>/dev/null
...
do
	...
	touch ../../messages/rls2_alive 2>/dev/null
		...
		then
			...
			then
				...
				do
					...
					if [[ $deadflag -eq 1 ]]
					then
						echo "checkid = "$id >> ./rls2_fulllog 2>/dev/null
						echo "thisid = "${targets[0+$((idx-1))*10]} >> ./rls2_fulllog 2>/dev/null
						echo "isalive = "${targets[5+$((idx-1))*10]} >> ./rls2_fulllog 2>/dev/null
						echo "isreported = "${targets[6+$((idx-1))*10]} >> ./rls2_fulllog 2>/dev/null
					...
					then
						...
						then
							...
							then
								echo "Обнаружена цель с ID = "$id" с координатами "$x" и "$y" и скоростями "$vx" и "$vy >>../../messages/rls2messages 2>/dev/null
								echo "`date +%s`:$id:$x:$y:0:-1" >> ../../messages/rls2_kplog 2>/dev/null
								...
								then
									...
									echo "id = "$id >> ./rls2_fulllog 2>/dev/null
									echo "targetInSproDir = "$targetInSproDir >> ./rls2_fulllog 2>/dev/null
									if [[ $targetInSproDir -eq 1 ]]
									then
										echo "Цель ID = "$id" движется в зону действия СПРО" >> ../../messages/rls2messages 2>/dev/null
										echo "`date +%s`:$id:$x:$y:3:-1" >> ../../messages/rls2_kplog 2>/dev/null
									fi
	...
	echo "unfound count = "$unfound_count >> ./rls2_fulllog 2>/dev/null
	
	# после полной итерации считывания целей обрабатываем новые цели	
	for ((thiscount=1; thiscount<=unfound_count; thiscount++))
	do
		echo "unfound id# "$thiscount" = "${unfound[0+$((thiscount-1))*3]} >> ./rls2_fulllog 2>/dev/null
	...
	echo $iter >> ./rls2_idlog
	$((iter++)) 2>/dev/null
		
	sleep 0.5

	echo "___" >> ./rls2_fulllog 2>/dev/null
	echo "NEW ITER"$iter >> ./rls2_fulllog 2>/dev/null
	echo "___" >> ./rls2_fulllog 2>/dev/null
